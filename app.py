...
def analyze_image(image):
    ...
    try:
        ...
        if predicted_class is not None:
            ...
            shape_badge = "🔴 Палочковидная" if bacteria_info['rod_shaped'] else "🟢 Шаровидная"
            
            return (
                f"🦠 **{bacteria_info['name']}**",
                f"📐 {bacteria_info['shape']}",
                f"💯 Уверенность: {confidence_percent:.1f}%",
                bacteria_info['description'],
                shape_badge
            )
        else:
...
def create_interface():
    with gr.Blocks(title="MicroDetect AI", theme=gr.themes.Soft()) as demo:
        ...
        Загрузите изображение бактерии и получите результат распознавания с помощью машинного обучения.
        """)
        
        with gr.Row():
            with gr.Column(scale=1):
                gr.Markdown("### 📸 Загрузите изображение")
                input_image = gr.Image(
                    label="Выберите изображение бактерии",
                    type="pil",
                    height=400
                )
            ...
            with gr.Column(scale=1):
                gr.Markdown("### 🎯 Результат распознавания")
                
                bacteria_name = gr.Markdown("**Ожидание загрузки...**")
                bacteria_shape = gr.Markdown("")
                confidence_text = gr.Markdown("")
                description_text = gr.Markdown("")
                shape_badge = gr.Markdown("")
        
        gr.Markdown("---")
        
        with gr.Accordion("📖 Информация о проекте", open=False):
            gr.Markdown("""
            ### 🌐 О MicroDetect AI
            ...
            **Поддерживаемые типы:**
            - 🦠 Стрептококки (шаровидные, цепочки)
            - 🦠 Стафилококки (шаровидные, гроздья)
            - 🦠 Сарцины (шаровидные, пакеты)
            - 🦠 Кишечные палочки (палочковидные)
            - 🦠 Палочковидные бактерии (палочковидные)
            ...
            **Точность модели:** 100% на тестовых данных
            """)
        ...
        analyze_btn.click(
            fn=analyze_image,
            inputs=[input_image],
            outputs=[bacteria_name, bacteria_shape, confidence_text, description_text, shape_badge]
        )
        
        # Автоматический анализ при загрузке изображения
        input_image.change(
            fn=analyze_image,
            inputs=[input_image],
            outputs=[bacteria_name, bacteria_shape, confidence_text, description_text, shape_badge]
